<script lang="ts">
  ...
  let items = $derived.by(() => {
    ...
    if (showOnlyLiked) {
      items = items.filter((item) => heartedItems.includes(item.search_item));
    }
      ...
      <TableBody tableBodyClass="divide-y">
        {#each items.slice(0, showAmount) as item, i}
          ...
          <TableBodyRow class="group">
            ...
            <TableBodyCell>
              {#if metadata?.image_path}
                ...
                <a href={metadata.url} target="_blank" id="icon-{i}">
                  <img
                    class="min-w-8 max-w-8 min-h-8 max-h-8 object-contain h-auto box-content bg-white p-0.5  rounded-md  opacity-70 group-hover:opacity-100"
                    src="{$page.data.api_url}{metadata.image_path}" alt={item.search_item} />
                </a>
              {/if}
            </TableBodyCell>
            <TableBodyCell>
            ...
            <TableBodyCell
            >
              <Popover border={false} color="navbar" triggeredBy="#price-{i}" class="text-sm pointer-events-none"
                       defaultClass="">
                <dl class="grid grid-cols-2 px-5 pt-2 pb-3 gap-y-1 text-slate-400 dark:text-slate-500">
                  <dt>count</dt>
                  <dd class="font-bold">{item.n_owled}</dd>
                  <hr class="col-span-2 -mx-5 my-0.5" />
                  <dt class:selected={selectedPrice === 'p0'}>min</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'p0'}>{item.p0.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'p25'}>p25</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'p25'}>{item.p25.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'p50'}>p50</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'p50'}>{item.p50.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'p75'}>p75</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'p75'}>{item.p75.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'p100'}>p100</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'p100'}>{item.p100.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'mean'}>mean</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'mean'}>{item.mean.toLocaleString()}</dd>
                  <dt class:selected={selectedPrice === 'std'}>std</dt>
                  <dd class=" font-bold" class:selected={selectedPrice === 'std'}>{item.std.toLocaleString()}</dd>
                </dl>
              ...
              <div class="flex justify-between gap-2">
                ...
                <a
                  ...
                  <ArrowUpRightFromSquareOutline size="sm" strokeWidth="2"
                                                 class="text-primary-500 inline-block relative -top-[0.1em]" />
                </a>
              </div>
            </TableBodyCell
            >
